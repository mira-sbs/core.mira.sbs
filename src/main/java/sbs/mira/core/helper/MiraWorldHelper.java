  ...
  extends MiraModule<MiraPulse<?, ?>>
{
  ...
  void breathe( )
  {
    this.repository = Path.of( Objects.requireNonNull(
      pulse()
        .plugin( )
        .getConfig( )
        .getString( "mira.file_path.world_repository" )
    ) );
   ...
   */
  public
  boolean stores( @NotNull World to_store, boolean allow_overwrite ) throws IOException, IllegalStateException
  {
    to_store.save( );
    ...
    this.pulse( ).master( ).files( ).copies(
      ...
      repository.resolve( world_label ),
      allow_overwrite
    );
   ...
   */
  public
  void remembers( @NotNull String repository_world_label, @NotNull String target_world_label )
  throws IOException
  {
    pulse( ).plugin( ).log(
    ...
    this.pulse( ).master( ).files( ).copies(
      repository.resolve( repository_world_label ),
